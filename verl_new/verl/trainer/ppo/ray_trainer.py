...
def _opd_trace_to_cpu(value):
    ...
    if isinstance(value, dict):
        return {key: _opd_trace_to_cpu(val) for key, val in value.items()}
    if isinstance(value, tuple):
        return tuple(_opd_trace_to_cpu(val) for val in value)
    if isinstance(value, list):
        return [_opd_trace_to_cpu(val) for val in value]
...
def _opd_trace_shape(value):
    if torch.is_tensor(value):
        return tuple(value.shape)
    if isinstance(value, np.ndarray):
        return tuple(value.shape)
    if isinstance(value, dict):
        return {key: _opd_trace_shape(val) for key, val in value.items()}
    if hasattr(value, "items"):
        try:
            return {key: _opd_trace_shape(val) for key, val in value.items()}
        except Exception:
...
def _opd_trace_jsonable(value):
    ...
    if isinstance(value, np.ndarray):
        return value.tolist()
    if isinstance(value, dict):
        return {str(key): _opd_trace_jsonable(val) for key, val in value.items()}
    if isinstance(value, (list, tuple)):
        return [_opd_trace_jsonable(val) for val in value]
    return value


def _dump_opd_metrics_jsonl(path: str, record: dict):
...
def _dump_opd_trace_batch(
    ...
    payload = {
        ...
        "trace_note": "Batch was generated by the current rollout policy before this actor update.",
        "dump_index": dump_index,
        ...
        "batch_keys": list(batch.batch.keys()) if batch.batch is not None else [],
        "non_tensor_keys": list(batch.non_tensor_batch.keys()),
        "batch_shapes": _opd_trace_shape(batch.batch) if batch.batch is not None else {},
    }
...
class RayPPOTrainer:
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        trace_dump_path = os.getenv("VERL_OPD_TRACE_DUMP")
        trace_dump_num_updates = int(os.getenv("VERL_OPD_TRACE_NUM_UPDATES", "1") or "1")
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                metrics_dump_path = os.getenv("VERL_OPD_METRICS_DUMP")
                if metrics_dump_path:
                    _dump_opd_metrics_jsonl(
                        metrics_dump_path,
                        {
                            "format": "verl_opd_metrics_v1",
                            "trace_stage": "post_actor_update",
