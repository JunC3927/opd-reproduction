...
class FSDPEngine(BaseEngine):
    ...
    def _capture_param_update_sample(self, max_numel: int = 8192):
        """
        Capture a small local parameter shard before optimizer.step for update diagnostics.
        """
        preferred = ("lm_head", "language_model", "model.layers", "embed_tokens")
        fallback = None
        ...
        name, param = fallback
        sample = param.detach().reshape(-1)[:max_numel].float().clone()
        return name, param, sample

    def _compute_param_update_metrics(self, sample) -> dict[str, float]:
        if sample is None:
            return {}

        _name, param, before = sample
        after = param.detach().reshape(-1)[: before.numel()].float()
        delta = (after - before.to(after.device)).abs()
        base = before.to(after.device).abs()
        max_abs = delta.max() if delta.numel() else torch.tensor(0.0, device=after.device)
        mean_abs = delta.mean() if delta.numel() else torch.tensor(0.0, device=after.device)
        rel_mean = delta.sum() / base.sum().clamp_min(1e-12)
        sampled_numel = torch.tensor(float(delta.numel()), device=after.device)

        if torch.distributed.is_available() and torch.distributed.is_initialized():
            torch.distributed.all_reduce(max_abs, op=torch.distributed.ReduceOp.MAX)
            torch.distributed.all_reduce(mean_abs, op=torch.distributed.ReduceOp.SUM)
            torch.distributed.all_reduce(rel_mean, op=torch.distributed.ReduceOp.SUM)
            torch.distributed.all_reduce(sampled_numel, op=torch.distributed.ReduceOp.SUM)
            world_size = torch.distributed.get_world_size()
            mean_abs = mean_abs / world_size
            rel_mean = rel_mean / world_size

        return {
            "param_update_max_abs": max_abs.item(),
            "param_update_mean_abs": mean_abs.item(),
            "param_update_rel_mean": rel_mean.item(),
            "param_update_sampled_numel": sampled_numel.item(),
        }
    ...
    def optimizer_step(self):
        ...
        update_sample = self._capture_param_update_sample()
        ...
        self._last_optimizer_step_metrics = self._compute_param_update_metrics(update_sample)
